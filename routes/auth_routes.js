...
router
  ...
  .post(async (req, res) => {
    ...
    const firstNameInput = xss(req.body.firstName);
    const lastNameInput = xss(req.body.lastName);
    const emailAddressInput = xss(req.body.email);
    let ageInput = xss(req.body.age);
    const stateInput = xss(req.body.state);
    const gradYearInput = xss(req.body.gradYr);
    const passwordInput = xss(req.body.password);
    const confirmPasswordInput = xss(req.body.confirmPassword);
    ...
    if (isNaN(ageInput) || ageInput < 0) {
      //Don't forget to convert into Number from String
    ...
    if (
      typeof gradYearInputNum !== "number" ||
      gradYearInputNum < 0 ||
      /^\d{4}$/.test(gradYearInput) === false
    ) {
    ...
    try {
      ...
      return res
        .status(400)
        .render("studentregister", { title: "Student Reigstration", error: e });
    }
  });
...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      if (newUser.insertedUser === true) {
        const newRecruiter = await recruiterData.createRecruiter(
          firstNameInput,
          lastNameInput,
          nEmailAddress,
          companyInput,
        ...
        if (newRecruiter.insertedRecruiter === true) {
          return res
            .status(201)
            .render("regsuccess", {
              title: "Recruiter Registration Successful",
            });
        } else {
          return res
            .status(400)
...
router
  ...
  .post(async (req, res) => {
    ...
    if (!emailAddressInput || !passwordInput) {
      ...
      return res
        .status(400)
        .render("login", { title: "Login", error: missingInputs });
    }
    let nEmailAddress;
    if (!validEmail(emailAddressInput)) {
    ...
    try {
      ...
      if (user.userFound === true) {
        ...
        if (isRecruiter) {
          return res
            .status(201)
            .render("landingPage", { title: "Recruiter Home" });
        } else if (isApplicant) {
          return res
            .status(201)
            .render("landingPage", { title: "Student Home" });
        }
      } else {
      ...
      return res
        .status(400)
        .render("login", { title: "Login", error: e.message || e });
    }
  });
...
router.route("/homepage").get(async (req, res) => {
  ...
  if (req.session.user.applicant) {
    try {
      jobsapplied = await applicantData.getJobsApplied(
        req.session.user.applicant._id
      );
    } catch (e) {}
    try {
      jobsfavorited = await applicantData.getJobsFavorited(
        req.session.user.applicant._id
      );
    } catch (e) {}
  }
  if (req.session.user.recruiter) {
    try {
      jobslisted = await jobData.getJobsByRecruiterId(
        req.session.user.recruiter._id
      );
    } catch (e) {}
  }
  res.render("homepage", {
...
router.route("/jobdetails/:id").get(async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      job.recruiterId == req.session.user.recruiter._id
    ) {
      try {
        applicants = await jobData.getJobApplicants(req.params.id);
      } catch (e) {}
    ...
    res
      .status(400)
      .render("error", { title: "Error In Job Details", error: e });
  }
});
  ...
  .post(async (req, res) => {
    ...
    if (!titleInput || !companyInput || !websiteInput || !tagsInput) {
      ...
      if (missingInputs.length > 0) {
        res
          .status(400)
          .render("jobcreate", { title: "Job Create", error: missingInputs });
        return;
      }
    }
    ...
    try {
      ...
      res
        .status(400)
        .render("error", { title: "Job Creation Error", error: e });
    }
  });
...
router.route("/apply/:id").get(async (req, res) => {
  ...
  try {
    ...
    if (job && job.applicants.includes(req.session.user.applicant._id))
      throw "User has already applied for this job";
    ...
    res
      .status(400)
      .render("error", { title: "Error In Apply Job Operation", error: e });
  }
});
...
router.route("/favorite/:id").get(async (req, res) => {
  ...
  try {
    if (!req.session.user.applicant) throw "Must be applicant to apply for job";
    let job = await jobData.getJob(req.params.id);
    let updated_applicant = await applicantData.get(
      req.session.user.applicant._id
    );

    if (updated_applicant.jobsFavorited.includes(job._id))
      throw "User has already favorited this job";

    await applicantData.favoriteJob(
      req.session.user.applicant._id,
      req.params.id
    );
    ...
    res
      .status(400)
      .render("error", { title: "Error In Favorite Job Operation", error: e });
  }
});
